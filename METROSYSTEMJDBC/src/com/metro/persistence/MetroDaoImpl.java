...
//		int swipedOut = MetroCards.getMetroCards().get(idOut).getSwipeOut();
	...
	public boolean swipeIn(int idIn, int stationId) throws ClassNotFoundException, SQLException {
		Connection connection=MySQLConnection.getConnection();
		PreparedStatement statement = connection.prepareStatement("update metrocards set swipein=? where id=?");
...
//		return MetroCards.getMetroCards().get(id);
	...
	@Override
	public boolean addCard(int id, int balance) throws SQLException, ClassNotFoundException {
		Connection connection=MySQLConnection.getConnection();
		PreparedStatement statement = connection.prepareStatement("insert into metrocards values(null, ?,0,0);");
...
//		MetroCards.getMetroCards().put(card.getId(), card);
	...
	@Override
	public Collection<MetroCard> getAllRecords() throws ClassNotFoundException, SQLException {
...
//		MetroCards.getMetroCards().get(id).setSwipeOut(0);
		Connection connection=MySQLConnection.getConnection();
		PreparedStatement statement = connection.prepareStatement("update metrocards set swipein=0, swipeout=0 where id=?");
		statement.setInt(1,id);
		statement.executeUpdate();
		connection.close();
	}
